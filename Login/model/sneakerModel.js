...
const getAllSneakers = async () => {            
	const [sneakers] = await promisePool.execute('SELECT * FROM user INNER JOIN shoes ON shoes.user = user.ID INNER JOIN malli ON shoes.malli = malli.malli_id; ')
	let mapped =sneakers.map(e => ({ hinta: e.hinta, koko: e.koko, nimi: e.nimi, Email: e.Email, kuva: e.kuva })); 
	console.log(mapped);
	return mapped;
}
...
const getMalli = async (name) => {
 ...
 const getUserByEmail = async (email) => {
	...
	const [kayttaja] = await promisePool.execute("SELECT * from user where Email = ? LIMIT 1;", [email]);
...
const createShoe = async (shoeData) => {
	...
	const [shoe] = await promisePool.execute("INSERT INTO `shoes` (`shoes_id`, `malli`, `hinta`, `koko`, `user`, `kuva`) VALUES (NULL, ?, ?, ?, ?, ?);", [malli.malli_id, shoeData.hinta, shoeData.koko, kayttaja.ID, kuva]);
	return shoe;
}
